...
# https://huggingface.co/docs/diffusers/advanced_inference/outpaint
...
def main(seed, device):
    ...
    pipeline_inpaint = StableDiffusionXLInpaintPipeline.from_pretrained(
        'OzzyGT/RealVisXL_V4.0_inpainting', torch_dtype=torch.float16, variant='fp16'
    ).to(device)

    context = ExperimentalContext(
        seed=seed, device=device, root_dir=os.path.join('out', 'sdxl_realvis_inpaint_sample')
    )
    inference(pipeline_inpaint=pipeline_inpaint, context=context, prompt=prompt, num_inference_steps=50)
